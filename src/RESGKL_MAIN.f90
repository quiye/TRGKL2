SUBROUTINE RESGKL_MAIN(MODE,ACCURACY,M,N,L,K,MATDESCRA,INDXA,PNTRBA,PNTREA,A)
  ...
  DOUBLE PRECISION A(*),cpbeta,te1,te2
  CHARACTER*6 MATDESCRA
  CHARACTER MODE

  INTEGER INFO,COU,SELEK,ACCURACY
  INTEGER I,STARTT,TMPT,MINT
  INTEGER ISEED(4),ITR
  ...
  DO SELEK = 1, 6!, 2
     ...
     DO WHILE (COU < 50)
        ...
        !WRITE(*,*) "ERR", SELEK,ITR,(TMPT-STARTT)+GKL_TIME,TMP_ERR,cpbk(1,1),cpbk(2,2),cpbk(3,3)
        IF(10**(0.0 - accuracy) > TMP_ERR) exit
        COU = COU + 1
     END DO
     WRITE(*,*) "ENDTIME", SELEK,ITR,(TMPT-STARTT)+GKL_TIME,TMP_ERR
     te2 = 0.0
     DO I = 1,L
        TE1 = ERR(mode,MATDESCRA,INDXA,PNTRBA,PNTREA,A,M,N,K,I,CPBK,CPVK,CPUK)
        te2 = te2 + te1
        write(*,*) TE1
     END DO
     write(*,*)
     ...
     DO I = 1,3
        WRITE (*,*) "Singular Value",I, CPBK(I,I)
     END DO
  END DO
