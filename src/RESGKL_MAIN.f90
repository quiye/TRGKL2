SUBROUTINE RESGKL_MAIN(ini,mtd,MODE,LS,ACCURACY,M,N,L,K,IAP,JA,A,WORK,LWORK)
  use omp_lib
  IMPLICIT NONE
  ...
  INTEGER ISEED(4), IAP(*), JA(*),ini,mtd
  ...
  DO W = 1,1
     ...
     !DO SELEK = 1, 1!, 2
        ...
        VPLUS = VPLUS_ORIG
        DO WHILE (COU < 5000)
           CALL RESGKL(I,mode,LS,IAP,JA,A,M,N,K,L,BK,VK,UK,VPLUS,INFO,SELEK,WORK,LWORK)
        ...
        TAVE = ERR(mode,IAP,JA,A,M,N,K,L,BK,VK,UK,WORK)
        TMAX = ERRMAX(mode,IAP,JA,A,M,N,K,L,BK,VK,UK,WORK)
